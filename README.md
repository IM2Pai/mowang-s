...
- 设置开镜模式、关闭按键、启动按键
- 保存压枪参数到第一把到第六把配置槽
- 压枪参数按最多 5 段写入；旧数据只有 3 段时，后 2 段会按 0 写入
- 保存后自动启用当前配置槽
...
- 当前页面按 5 段打包，配置负载为 58B。前 36B 保持原 3 段格式不变：开头 1B 是启动按键，`0x22-0x23` 是数据 ID。
- 第 4 段从 `0x24` 开始，第 5 段从 `0x2F` 开始，用来尽量兼容固件对旧 3 段结构的读取逻辑。
